...
export default function BillsTable({ bills, loading, error }: Props) {
  return (
    <div className="overflow-x-auto max-w-full">
      <table className="w-full table-auto border-collapse divide-y divide-mb-border">
        <thead className="bg-mb-brand">
          <tr>
            {['Type', 'Bill No.', 'Title / Subject', 'Date Filed', 'Author', 'Action'].map(col => (
              <th
                key={col}
        ...
        <tbody>
          ...
          {!loading && error && (
            <tr>
              <td colSpan={6} className="py-16 text-center text-red-600 text-sm">
                Error: {error}
              </td>
            </tr>
          )}
          ...
          {!loading && !error && bills.map((bill, i) => (
            <BillsTableRow key={bill.id} bill={bill} isEven={i % 2 === 0} />
          ))}
        </tbody>
      </table>
    </div>
